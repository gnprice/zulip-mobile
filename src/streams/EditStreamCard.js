...
type Props = $ReadOnly<{|
  ...
  initialValues: {|
    ...
    description: string,
    invite_only: boolean,
    is_web_public: boolean | void,
    history_public_to_subscribers: boolean,
  |},
  onComplete: (name: string, description: string, policySettings: StreamPolicySettings) => void,
|}>;
...
export default class EditStreamCard extends PureComponent<Props, State> {
  state: State = {
    ...
    accessPolicy: accessPolicyFromStreamSettings({
      invite_only: this.props.initialValues.invite_only,
      is_web_public: this.props.initialValues.is_web_public ?? false,
      history_public_to_subscribers: this.props.initialValues.history_public_to_subscribers,
    }),
  };
