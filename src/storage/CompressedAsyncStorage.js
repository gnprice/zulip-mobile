...
import { AsyncStorageImpl, Migration } from './AsyncStorage';
...
export class CompressedAsyncStorageImpl {
  ...
  constructor(version: number, migrations: $ReadOnlyArray<Migration | CompressedMigration>) {
    this.storage = new AsyncStorageImpl(
      version,
  ...
  removeItem: (key: string) => Promise<void> = key => this.storage.removeItem(key);
